...
router.post("/slots", async (req, res) => {
    try {
        ...
        while (slots.length < totalNeeded) {
            ...
            // FIX #7: Skip BOTH Saturday (6) and Sunday (0)
            if (currentSlotTime.getDay() === 6) {          // Saturday → Monday
                currentSlotTime.setDate(currentSlotTime.getDate() + 2);
            } else if (currentSlotTime.getDay() === 0) {   // Sunday → Monday
...
router.post("/appointments/:id/review", async (req, res) => {
    try {
        const { id } = req.params;
        const { action, doctorId, doctorName, modifiedTherapy, modifiedDates, modifiedSessions, modifiedDatetime } = req.body;
...
router.post("/intake", async (req, res) => {
    try {
        ...
        // 3. Create session documents for each scheduled slot
        const sessionIds = [];
        ...
        for (let i = 0; i < slots.length; i++) {
            ...
            const sessionDoc = {
                ...
                practitioner_id: doctorId || null,   // FIX #8: was missing — caused sessions to be invisible to doctor dashboard
                doctor_name: doctorName || null,     // FIX #8
                ...
                priority: recommendation?.totalPriorityScore || recommendation?.priority_score || 50,
        ...
        // 4. Send confirmation email if email is available
        if (patientEmail) {
            try {
