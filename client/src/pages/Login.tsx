...
// Steps: role → (doctor-code) → auth-method → details → done
type SignupStep = "role" | "doctor-code" | "auth-method" | "details";
...
const Login = () => {
  ...
  const { user, login, loginWithGoogle, registerPatient, registerDoctor, updateUserProfile } =
    useAuth();
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Redirect after a brief delay to allow auth state to settle
  ...
  const goToNextStep = () => {
    if (signupStep === "role") {
      ...
      setDoctorCodeError("");
      setSignupStep("auth-method");
    } else if (signupStep === "auth-method") {
  ...
  const handleSignup = async (e: React.FormEvent) => {
    ...
    try {
      if (authMethod === "google") {
        // Google sign-up: create auth, then save profile
        await loginWithGoogle();
        // After Google auth, update the Firestore profile with collected details
        ...
        if (selectedRole === "doctor") {
          ...
          await registerPatient(signupEmail, signupPassword, name);
        }
      ...
      toast({
        ...
        description: `Welcome to AyurSutra, ${name}!`,
      });
  ...
  const totalSteps = selectedRole === "doctor" ? 4 : 3;
            ...
            <div className="animate-fade-in">
                ...
                <div className="space-y-4">
                  ...
                  <div className="space-y-3">
                    <button
                      ...
                      className={`w-full p-4 rounded-xl border-2 flex items-center gap-3 transition-all ${authMethod === "google"
                    ...
                    <button
                      ...
                      className={`w-full p-4 rounded-xl border-2 flex items-center gap-3 transition-all ${authMethod === "email"
                        ? "border-primary bg-primary/5"
                        : "border-muted hover:border-primary/40"
                        }`}
                    >
              ...
              {signupStep === "details" && (
                <form onSubmit={handleSignup} className="space-y-4">
                  ...
                  {/* Email & password (only if email method) */}
                  {authMethod === "email" && (
                    <>
                      <div className="space-y-2">
